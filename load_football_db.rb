...
seasonDataResponse = HTTParty.get("http://www.football-data.org/alpha/soccerseasons", :headers => {"Authorization" => "Token token='#{ENV['FOOTBALL_DATA_API_KEY']}'"})
seasonLeagues = JSON.parse(seasonDataResponse.body)

if seasonLeagues["error"] == nil
  seasonLeagues.each do |league|
    ...
    leagueDataResponse = HTTParty.get(leagueTeamsURL, :headers => {"Authorization" => "Token token='#{ENV['FOOTBALL_DATA_API_KEY']}'"})
    ...
    if leagueTeams != nil
      leagueTeams.each do |team|
        ...
        teamDataResponse = HTTParty.get(teamPlayersURL, :headers => {"Authorization" => "Token token='#{ENV['FOOTBALL_DATA_API_KEY']}'"})
        ...
        if teamPlayers != nil
          teamPlayers.each do |player|
            ...
            playerJerseyNumber = player["jerseyNumeber"]
            ...
            playerInsertQuery =  "INSERT INTO transferbreak_players (name, id, team, league, position, jersey_number, date_of_birth, nationality, contract_expires, market_value) VALUE ('#{playerName}', '#{playerId}', '#{teamName}', '#{leagueName}', '#{playerPosition}', '#{playerJerseyNumber}', '#{playerDateOfBirth}', '#{playerNationality}', '#{playerContractExpires}', '#{playerMarketValue}')"
          end
        end
      end
    end
